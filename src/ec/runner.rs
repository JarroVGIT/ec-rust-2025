...
fn check_and_submit<T: Display>(result: &Option<T>, quest: u8, part: u8) -> Option<String> {
    ...
    let should_submit = args.iter().position(|x| x == "--submit")
    ...
    match super::client::Client::new() {
        Ok(client) => {
            match client.submit_answer(quest, part, &result.to_string()) {
                Ok(response) => format_submission_response(&response),
                Err(e) => Some(format!("{}✗ Submission failed: {}{}", ANSI_RED, e, ANSI_RESET))
            }
        }
...
fn format_submission_response(response: &str) -> Option<String> {
    ...
    if let Ok(json) = serde_json::from_str::<serde_json::Value>(response) {
        let correct = json.get("correct").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        if correct {
            ...
            let length_correct = json.get("lengthCorrect").and_then(|v| v.as_bool()).unwrap_or(false);
            if !length_correct {
