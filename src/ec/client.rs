    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ClientError::SessionNotFound => {
                write!(f, "Session cookie not found. Please create ~/.ec-session with your session cookie value.")
            }
            ClientError::SeedNotConfigured => {
                write!(f, "EC_SEED not configured. Add it to .cargo/config.toml:\n[env]\nEC_SEED = \"your_seed_here\"")
            }
            ClientError::EventNotConfigured => {
                write!(f, "EC_EVENT not configured. Add it to .cargo/config.toml:\n[env]\nEC_EVENT = \"2024\"")
            }
...
impl Client {
    pub fn new() -> Result<Self, ClientError> {
        ...
        let seed = match Self::get_seed() {
            ...
            Err(_) => {
                ...
                println!("\n INFO: Fetched your seed from the API: {}. You can ", fetched_seed);
    ...
    fn get_seed() -> Result<u32, ClientError> {
        let seed_str = std::env::var("EC_SEED")
    ...
    fn get_event() -> Result<String, ClientError> {
        std::env::var("EC_EVENT")
            .map_err(|_| ClientError::EventNotConfigured)
    }
    ...
    pub fn fetch_user_seed(&self) -> Result<u32, ClientError> {
        ...
        if !response.status().is_success() {
            return Err(ClientError::HttpError(format!("Status: {}", response.status())));
        }
    ...
    pub fn fetch_encrypted_input(&self, quest: u8, part: u8) -> Result<String, ClientError> {
        ...
        let encrypted = inputs
            .get(part.to_string())
            .and_then(|v| v.as_str())
            .ok_or_else(|| ClientError::HttpError(format!("Part {} not found in response", part)))?;
    ...
    pub fn fetch_decryption_key(&self, quest: u8, part: u8) -> Result<String, ClientError> {
        ...
        key.ok_or_else(|| ClientError::HttpError(format!("Key for part {} not available (possibly not solved yet)", part)))
    }
    ...
    pub fn decrypt_input(&self, encrypted_hex: &str, key: &str) -> Result<String, ClientError> {
        ...
        if key_bytes.len() != 32 {
            return Err(ClientError::DecryptionError(format!("Key must be 32 bytes, got {}", key_bytes.len())));
        }
    ...
    pub fn submit_answer(&self, quest: u8, part: u8, answer: &str) -> Result<String, ClientError> {
        ...
        if status.is_success() {
            ...
            Err(ClientError::HttpError(format!("Submit failed ({}): {}", status, body)))
        }
    }
